...
C_range = [1, 2, 3]
os.makedirs(f"harmonica2degree", exist_ok=True)


for test_index in pbar:
    os.makedirs(f"harmonica2degree/{test_index}", exist_ok=True)
    ...
    for subspace_limit in [0, 1, 2, 4, 8]:
        ...
        np.save(
            f"harmonica2degree/{test_index}/testindex{test_index}_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
            scikit_lasso_result)
        np.save(
            f"harmonica2degree/{test_index}/testindex{test_index}_final_model_output_subspace{subspace_limit}_seed{args.seed}",
    ...
    for C in C_range:
        ...
        for subspace_limit in [0, 1, 2, 4, 8]:
            ...
            np.save(
                f"harmonica2degree/{test_index}/testindex{test_index}_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                scikit_lasso_result)
            np.save(
                f"harmonica2degree/{test_index}/testindex{test_index}_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                model_truthful_values)
            np.save(
                f"harmonica2degree/{test_index}/testindex{test_index}_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                sigmas)
            np.save(
                f"harmonica2degree/{test_index}/testindex{test_index}_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                answer)
