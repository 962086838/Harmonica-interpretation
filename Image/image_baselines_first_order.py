...
parser.add_argument('--datapath', metavar='DIR', nargs='?', default='/mnt/sda1/hehaowei/ImageNet/val/',
...
if args.algorithm == "lime":
    ...
    for test_index in pbar:
        os.makedirs(f"lime/{test_index}", exist_ok=True)
        ...
        for subspace_limit in [0, 1, 2, 4, 8]:
            ...
            np.save(
                f"lime/{test_index}/testindex{test_index}_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                lime_result)
            np.save(
                f"lime/{test_index}/testindex{test_index}_final_model_output_subspace{subspace_limit}_seed{args.seed}",
        ...
        for C in C_range:
            ...
            for subspace_limit in [0, 1, 2, 4, 8]:
                ...
                np.save(
                    f"lime/{test_index}/testindex{test_index}_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    lime_result)
                np.save(
                    f"lime/{test_index}/testindex{test_index}_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    model_truthful_values)
                np.save(
                    f"lime/{test_index}/testindex{test_index}_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    sigmas)
                np.save(
                    f"lime/{test_index}/testindex{test_index}_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    answer)
    ...
    C_range = [1, 2, 3]
    for test_index in pbar:
        os.makedirs(f"ig/{test_index}", exist_ok=True)
        ...
        for subspace_limit in [0, 1, 2, 4, 8]:
            ...
            np.save(f"ig/{test_index}/testindex{test_index}_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                    ig_result)
            np.save(f"ig/{test_index}/testindex{test_index}_final_model_output_subspace{subspace_limit}_seed{args.seed}",
                    model_truthful_values)
        ...
        for C in C_range:
            ...
            for subspace_limit in [0, 1, 2, 4, 8]:
                ...
                np.save(
                    f"ig/{test_index}/testindex{test_index}_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    ig_result)
                np.save(
                    f"ig/{test_index}/testindex{test_index}_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    model_truthful_values)
                np.save(
                    f"ig/{test_index}/testindex{test_index}_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    sigmas)
                np.save(
                    f"ig/{test_index}/testindex{test_index}_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    answer)
    ...
    C_range = [1, 2, 3]
    for test_index in pbar:
        os.makedirs(f"ks/{test_index}", exist_ok=True)
        ks = KernelShap(model)
        ...
        for subspace_limit in [0, 1, 2, 4, 8]:
            ...
            np.save(
                f"ks/{test_index}/testindex{test_index}_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                ks_result)
            np.save(
                f"ks/{test_index}/testindex{test_index}_final_model_output_subspace{subspace_limit}_seed{args.seed}",
                model_truthful_values)
        ...
        for C in C_range:
            ...
            for subspace_limit in [0, 1, 2, 4, 8]:
                ...
                np.save(
                    f"ks/{test_index}/testindex{test_index}_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    ks_result)
                np.save(
                    f"ks/{test_index}/testindex{test_index}_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    model_truthful_values)
                np.save(
                    f"ks/{test_index}/testindex{test_index}_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    sigmas)
                np.save(
                    f"ks/{test_index}/testindex{test_index}_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    answer)
