...
assert args.algorithm in ["ih", "shaptaylor"]
...
if args.split_end - args.split_start == 0 and args.split_start == 0:
    ...
    assert args.algorithm in ["ih", "shaptaylor"], "Other algorithms do not support acceleration"
...
if args.algorithm == "ih":
    ...
    for test_index in pbar:
        os.makedirs(f"ih/{test_index}", exist_ok=True)
        ...
        for subspace_limit in [0, 1, 2, 4, 8]:
            ...
            np.save(
                f"ih/{test_index}/testindex{test_index}_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                ih_result)
            np.save(
                f"ih/{test_index}/testindex{test_index}_final_model_output_subspace{subspace_limit}_seed{args.seed}",
        ...
        for C in C_range:
            ...
            for subspace_limit in [0, 1, 2, 4, 8]:
                ...
                np.save(
                    f"ih/{test_index}/testindex{test_index}_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    ih_result)
                np.save(
                    f"ih/{test_index}/testindex{test_index}_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    model_truthful_values)
                np.save(
                    f"ih/{test_index}/testindex{test_index}_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    sigmas)
                np.save(
                    f"ih/{test_index}/testindex{test_index}_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    answer)
    ...
    for test_index in pbar:
        os.makedirs(f"shaptaylor/{test_index}", exist_ok=True)
        ...
        for subspace_limit in [0, 1, 2, 4, 8]:
            ...
            np.save(
                f"shaptaylor/{test_index}/testindex{test_index}_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                shaptaylor_result)
            np.save(
                f"shaptaylor/{test_index}/testindex{test_index}_final_model_output_subspace{subspace_limit}_seed{args.seed}",
                model_truthful_values)
        ...
        for C in C_range:
            ...
            for subspace_limit in [0, 1, 2, 4, 8]:
                ...
                np.save(
                    f"shaptaylor/{test_index}/testindex{test_index}_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    shaptaylor_result)
                np.save(
                    f"shaptaylor/{test_index}/testindex{test_index}_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    model_truthful_values)
                np.save(
                    f"shaptaylor/{test_index}/testindex{test_index}_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    sigmas)
                np.save(
                    f"shaptaylor/{test_index}/testindex{test_index}_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    answer)
